...
it('asserts flags are correctly set')
    ->expect(fn (): CommandCall => new CommandCall(['minicli', 'help', 'test', '--flag']))
...
it('asserts params are correctly set')
    ->expect(fn (): CommandCall => new CommandCall(['minicli', 'help', 'test', 'name=test']))
    ->hasParam('name')->toBeTrue()
    ->getParam('name')->toBe('test');

it('asserts params are correctly set if value contains "="')
    ->expect(fn (): CommandCall => new CommandCall(['minicli', 'help', 'test', 'name=first=john&last=doe']))
    ->hasParam('name')->toBeTrue()
