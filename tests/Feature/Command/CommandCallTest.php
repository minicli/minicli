...
it('asserts input arguments are loaded and properties are set')
    ->expect(fn (): \Minicli\Command\CommandCall => new CommandCall(['minicli', 'help', 'test']))
    ->getRawArgs()->toHaveCount(3)
    ->command->toBe('help')
    ->subcommand->toBe('test');

it('asserts flags are correctly set')
    ->expect(fn (): \Minicli\Command\CommandCall => new CommandCall(['minicli', 'help', 'test', '--flag']))
...
it('asserts params are correctly set')
    ->expect(fn (): \Minicli\Command\CommandCall => new CommandCall(['minicli', 'help', 'test', 'name=test']))
    ->hasParam('name')->toBeTrue()
    ->getParam('name')->toBe('test');

it('asserts params are correctly set if value contains "="')
    ->expect(fn (): \Minicli\Command\CommandCall => new CommandCall(['minicli', 'help', 'test', 'name=first=john&last=doe']))
    ->hasParam('name')->toBeTrue()
