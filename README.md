...
There are several prebuilt commands, notably "help".
...
#### Register your commands into the help command.

To make sure that your own commands are listed in the autogenerated help, add their name, signature,
and optional description in the `$app->registerCommand()`:

```php
$app->registerCommand('greet', function (CommandCall $cli) use ($app, $commandsSummaries) {
    $args = $cli->getRawArgs();
    $name = $args[2] ?? 'World';
    echo "Hello, $name!\n";
...
minicli [required] <optional> - Description
help          — Lists the available commands.
greet <name>  — Emits a salutation
...
#### Extend prebuilt commands

You can extend any prebuilt command's actions and output by registering a callback function:

```php
$app->prebuilt->help->registerCallback(function (string $key = 'OTP') {
